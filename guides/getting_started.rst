...
::
    1. unzip it
    2. $ chmod +x start_private.sh //to make it executable 
    3. $ ./start_self.sh
...
1. ``$ ./start_self.sh``
start fractal node , if you shut down your PC ,you can run this to get fractal node run again

2. ``$ ./start_self.sh del`` 
it deletes all files to restore the original state

3. ``$ ./start_self.sh check``
it check if the fractal node runs well

4. ``$ ./start_self.sh stop`` 
it stops fractal node ,shut it down
